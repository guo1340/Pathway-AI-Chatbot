...
- The header now uses an information button. Its dialog contains answer guidance, current input/max-response token details, the known daily balance, and the clear-history action. Token help question marks belong inside the token grid header cells beside `Input tokens` and `Max response`, not beside the `Token usage` section title.
